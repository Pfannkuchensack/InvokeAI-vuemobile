...
export const useModelssd1Store = defineStore('modelssd1', () => {
	...
	*/
	function invoke(state: { ip: string, port: string}, prompt: { positive: string, negative: string }, config: { rseed: boolean, seed: number, model: string, clip: number, width: number, height: number, cfg: number, steps: number, scheduler: string, iterations: number}, preview: boolean) {
		// make api request
		if (preview) {
